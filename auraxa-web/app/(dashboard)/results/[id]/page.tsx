...
const ICONS: Record<string, string> = {
  ...
  card:        "M2 5a2 2 0 012-2h16a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5zM2 10h20",
  ...
  flame:       "M8.5 14.5A2.5 2.5 0 0011 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 01-14 0c0-1.045.5-2 1-3a2.5 2.5 0 002.5 2.5z",
  layers:      "M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5",
  sparkle:     "M12 3l1.5 4.5L18 9l-4.5 1.5L12 15l-1.5-4.5L6 9l4.5-1.5z",
  bubbles:     "M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z",
  ...
  userX:       "M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8M18 8l4 4m0-4l-4 4",
  link:        "M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71",
  eye:         "M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8zM12 9a3 3 0 100 6 3 3 0 000-6z",
  brain:       "M9 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2h-4M9 3a3 3 0 006 0M9 3h6",
  activity:    "M22 12h-4l-3 9L9 3l-3 9H2",
  shield:      "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z",
  compass:     "M12 22a10 10 0 100-20 10 10 0 000 20zM16.24 7.76l-1.5 5.5-5.5 1.5 1.5-5.5 5.5-1.5z",
};

function Icon({ name, size = 15, color = "currentColor", sw = 1.75 }: { name: string; size?: number; color?: string; sw?: number }) {
...
// ─── Types ────────────────────────────────────────────────
interface Scores {
  overall_score: number; compatibility_score: number; communication_balance: number;
  speaker_a_percentage: number; speaker_b_percentage: number; toxicity_level: string;
  ghosting_risk: string; attachment_style?: string; patterns_detected: string[]; ai_narrative?: string;
}
interface AnalysisResult {
  id: string; status: string; speakers?: { a?: string; b?: string }; scores?: Scores;
  ai_narrative?: string; message_count?: number; date_range?: any; genz_verdict?: string;
  timeline?: any[]; hard_truths?: string[]; key_topics?: any[]; conversation_themes?: any;
  conversation_phases?: any[]; communication_analysis?: any; red_flags?: any[]; green_flags?: any[];
  emotional_moments?: any; relationship_health_indicators?: any; roast?: any; astrology_reading?: any;
  what_this_reveals?: string; therapist_note?: string; scoring_breakdown?: any; sub_metrics?: any; peak_moments?: any;
}

// ─── Helpers ──────────────────────────────────────────────
const SC: Record<string, string> = { low: "var(--green)", medium: "var(--amber)", high: "var(--red)", critical: "var(--red)" };
const SENT_C: Record<string, string> = { positive: "var(--green)", negative: "var(--red)", neutral: "var(--muted)", mixed: "var(--amber)" };
const PHASE_C: Record<string, string> = { red: "var(--red)", green: "var(--green)", neutral: "var(--amber)" };
...
const VARIANT_CONFIG: Record<string, { label: string; color: string }> = {
  slay: { label: "Slay Era", color: "#6c55e0" }, healing: { label: "Healing Arc", color: "#047857" },
  mid: { label: "Mid Energy", color: "#b45309" }, cooked: { label: "Cooked", color: "#cc0000" },
};
...
function StatusDot({ level }: { level: string }) {
  ...
  return (
    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full flex-shrink-0" style={{ background: `${c}18`, border: `1.5px solid ${c}` }}>
...
function ScoreRing({ score }: { score: number }) {
  const size = 140; const r = size * 0.41; const circ = 2 * Math.PI * r; const offset = circ - (score / 100) * circ;
  return (
    ...
    <div className="space-y-1.5">
      ...
      <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--surface-alt)" }}>
        <motion.div className="h-full rounded-full" style={{ background: color }} initial={{ width: 0 }}
          animate={{ width: `${clamped}%` }} transition={{ duration: 1.2, delay, ease: [0.16, 1, 0.3, 1] }} />
      </div>
    </div>
  );
}
...
function Section({ title, iconName, iconColor = "var(--primary)", accent = false, children }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5 }} className="card overflow-hidden mb-4">
      <div className="px-4 sm:px-6 py-3 flex items-center gap-2.5" style={{ borderBottom: "1px solid var(--line)", background: accent ? "var(--pri-soft)" : "var(--surface)" }}>
...
function ScoreCard({ result }: { result: AnalysisResult }) {
  ...
  const bg = bgMap[variant] ?? "#f8f7ff"; const clr = clrMap[variant] ?? "#6c55e0";
  ...
  const download = async () => {
    ...
    try {
      ...
      const a = document.createElement("a"); a.download = `auraxa-${s.overall_score}.png`; a.href = canvas.toDataURL("image/png"); a.click();
      toast.success("Saved!");
  ...
  const share = async () => {
    ...
    try { if (navigator.share) await navigator.share({ text, url: window.location.href }); else { await navigator.clipboard.writeText(text); toast.success("Copied!"); } } catch { }
  };
    ...
    <div>
      <div ref={cardRef} style={{ background: bg, border: "1px solid rgba(0,0,0,.08)", borderRadius: "10px", padding: "24px", maxWidth: "320px", fontFamily: "'Inter',sans-serif" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
          ...
          <div style={{ fontSize: "9px", color: "rgba(0,0,0,.28)", fontFamily: "'Montserrat',sans-serif", fontWeight: 600, letterSpacing: "0.06em" }}>{(result.speakers?.a ?? "You").toUpperCase()} x {(result.speakers?.b ?? "Them").toUpperCase()}</div>
        </div>
      ...
      <div className="flex gap-2 mt-3" style={{ maxWidth: "320px" }}>
        <button onClick={download} disabled={downloading} className="btn btn-primary flex-1" style={{ fontSize: "11px", padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
          {downloading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Icon name="download" size={12} color="#fff" />Save Card</>}
        </button>
        <button onClick={share} className="btn btn-secondary" style={{ fontSize: "11px", padding: "10px 14px", display: "flex", alignItems: "center", gap: "6px" }}>
          <Icon name="share" size={12} color="var(--primary)" />Share
        </button>
      </div>
    </div>
  );
...
// Timeline removed — not rendering

function ProcessingView({ pollCount }: { pollCount: number }) {
  const STEPS = ["Extracting text", "Structuring messages", "Running AI analysis", "Calculating sub-metrics", "Finding phases and peaks", "Writing hard truths", "Consulting the cosmos"];
        ...
        <div className="mb-8 space-y-3">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: i <= step ? "var(--primary)" : "var(--surface-alt)", transition: "background .4s" }}>
                {i <= step && <Icon name="check" size={10} color="#fff" sw={3} />}
              </div>
              ...
              <p className="text-xs font-display font-bold" style={{ color: i <= step ? "var(--text)" : "var(--muted)", minWidth: "160px", textAlign: "left" }}>{s}</p>
            </div>
          ))}
        </div>
        <p className="label text-center" style={{ fontSize: "10px" }}>Reading the unsaid... (~30-60 seconds)</p>
      </div>
    </div>
  );
}
...
export default function ResultsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (status === "completed") {
        ...
        if (pollCount >= 40) { setResult({ id, status: "failed" } as any); return; }
        setResult((prev) => prev ? { ...prev, status } : { id, status } as any);
        setPollCount((c) => c + 1);
        setTimeout(load, 3000);
      }
    } catch (e: any) {
      if (e?.response?.status === 404) { setResult({ id, status: "failed" } as any); }
      else if (pollCount < 30) { setTimeout(load, 4000); }
  ...
  if (loading && !result) {
    return <div className="flex items-center justify-center min-h-[80vh]"><div className="w-12 h-12 rounded-full border-2 animate-spin" style={{ borderColor: "var(--line-2)", borderTopColor: "var(--primary)" }} /></div>;
  }
      ...
      <div className="flex items-center justify-center min-h-[80vh] px-6">
        <div className="card p-10 text-center max-w-sm">
          <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: "var(--red-soft)" }}>
            <Icon name="alertTri" size={22} color="var(--red)" />
          </div>
  ...
  const sub = result?.sub_metrics; const sb = result?.scoring_breakdown;
  const peaks = result?.peak_moments; const phases = result?.conversation_phases ?? [];
  const topics = result?.key_topics ?? []; const comm = result?.communication_analysis;
  const redFlags = result?.red_flags ?? []; const greenFlags = result?.green_flags ?? [];
  const hi = result?.relationship_health_indicators; const roast = result?.roast;
  const astro = result?.astrology_reading; const hardTruths = result?.hard_truths ?? [];
    ...
    <div className="px-3 sm:px-5 md:px-8 py-4 md:py-8 max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-2">
        <Link href="/reports" className="inline-flex items-center gap-1.5 label hover:text-primary transition-colors" style={{ fontSize: "9px" }}>
          <Icon name="back" size={12} color="var(--muted)" />Back to Reports
        </Link>
        <div className="flex items-center gap-2 flex-wrap">
          <Link href={`/advisor/${id}`} className="btn btn-secondary inline-flex items-center gap-2" style={{ fontSize: "11px", padding: "8px 14px" }}>
            <Icon name="message" size={12} color="var(--primary)" />AI Advisor
          </Link>
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded mb-2" style={{ background: "var(--pri-soft)", border: "1px solid var(--pri-border)" }}>
                <div className="w-1.5 h-1.5 rounded-full" style={{ background: vc.color }} />
      ...
      <Section title="Your Score Card" iconName="card"><ScoreCard result={result!} /></Section>

      {sb && (
        <Section title="Scoring Breakdown" iconName="barChart">
      ...
      {sub && (
        <Section title="Sub-Metrics" iconName="trendUp">
          <div className="space-y-5">
            {sub.initiation_balance && (
              <div>
                ...
                {sub.initiation_balance.note && <p className="text-xs leading-relaxed" style={{ color: "var(--muted)" }}>{sub.initiation_balance.note}</p>}
              </div>
            )}
            {sub.response_time_trend && (
              <div className="p-3 rounded" style={{ background: "var(--surface-alt)" }}>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <span className="px-2.5 py-1 rounded font-display font-bold capitalize" style={{ fontSize: "11px", background: sub.response_time_trend.trend === "improving" ? "var(--green)" : sub.response_time_trend.trend === "declining" ? "var(--red)" : "var(--amber)", color: "#fff" }}>
                    {sub.response_time_trend.trend}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  {sub.response_time_trend.person_a_trend && <div className="p-2 rounded" style={{ background: "var(--bg)" }}><p className="label" style={{ fontSize: "8px" }}>{speakerA.toUpperCase()}</p><p className="text-xs capitalize" style={{ color: "var(--text)" }}>{sub.response_time_trend.person_a_trend}</p></div>}
                  {sub.response_time_trend.person_b_trend && <div className="p-2 rounded" style={{ background: "var(--bg)" }}><p className="label" style={{ fontSize: "8px" }}>{speakerB.toUpperCase()}</p><p className="text-xs capitalize" style={{ color: "var(--text)" }}>{sub.response_time_trend.person_b_trend}</p></div>}
                </div>
                {sub.response_time_trend.note && <p className="text-xs leading-relaxed" style={{ color: "var(--muted)" }}>{sub.response_time_trend.note}</p>}
              </div>
            )}
            {sub.sentiment_arc && (
              <div>
                ...
                <div className="flex items-stretch gap-2 mb-2">
                  {[{ l: "Early", v: sub.sentiment_arc.early_sentiment }, { l: "Middle", v: sub.sentiment_arc.middle_sentiment }, { l: "Recent", v: sub.sentiment_arc.recent_sentiment }].map((a) => (
                    <div key={a.l} className="flex-1 p-2.5 rounded text-center" style={{ background: `${SENT_C[a.v] ?? "var(--muted)"}15`, border: `1px solid ${SENT_C[a.v] ?? "var(--line)"}40` }}>
                ...
                {sub.sentiment_arc.arc_direction && <p className="text-xs font-bold capitalize mb-1" style={{ color: sub.sentiment_arc.arc_direction === "improving" ? "var(--green)" : sub.sentiment_arc.arc_direction === "declining" ? "var(--red)" : "var(--amber)" }}>Arc: {sub.sentiment_arc.arc_direction}</p>}
                {sub.sentiment_arc.note && <p className="text-xs leading-relaxed" style={{ color: "var(--muted)" }}>{sub.sentiment_arc.note}</p>}
              </div>
            )}
              ...
              <div className="p-3 rounded" style={{ background: "var(--surface-alt)" }}>
                ...
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-12 h-12 rounded flex items-center justify-center" style={{ background: "var(--pri-soft)" }}>
                    <span className="font-display font-bold text-xl" style={{ color: "var(--primary)" }}>{sub.affection_signals.count ?? 0}</span>
                  </div>
                  <div>
                    <p className="font-display font-bold text-sm" style={{ color: "var(--text)" }}>Affectionate moments</p>
                    <p className="label" style={{ fontSize: "9px" }}>Quality: <span className="capitalize">{sub.affection_signals.quality}</span> — mainly from {sub.affection_signals.who_shows_more}</p>
                  </div>
                </div>
                {sub.affection_signals.examples?.length > 0 && <div className="flex flex-wrap gap-1.5 mb-2">{sub.affection_signals.examples.map((e: string) => <span key={e} className="px-2 py-0.5 rounded text-xs" style={{ background: "var(--bg)", color: "var(--text)" }}>{e}</span>)}</div>}
                {sub.affection_signals.note && <p className="text-xs leading-relaxed" style={{ color: "var(--muted)" }}>{sub.affection_signals.note}</p>}
              </div>
            )}
          </div>
        </Section>
      ...
      {result?.ai_narrative && <Section title="The Honest Truth" iconName="book"><p className="text-sm leading-relaxed" style={{ color: "var(--muted)", whiteSpace: "pre-line" }}>{result.ai_narrative}</p></Section>}

      {hardTruths.length > 0 && (
        <Section title="Hard Truths" iconName="flame" iconColor="var(--red)">
          ...
          <div className="space-y-3">
            {hardTruths.map((t: string, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }} className="flex gap-3 p-3 rounded" style={{ background: "rgba(204,0,0,.06)", border: "1px solid rgba(204,0,0,.15)" }}>
      ...
      {phases.length > 0 && (
        <Section title="How This Evolved" iconName="layers">
          ...
          <div className="relative">
            ...
            <div className="space-y-5">
              {phases.map((ph: any, i: number) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex gap-4 pl-8 relative">
                    <div className="absolute left-0 top-0 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 font-display font-bold text-xs" style={{ background: `${phColor}18`, border: `2px solid ${phColor}`, color: phColor, zIndex: 1 }}>{ph.phase_number}</div>
                    <div className="flex-1 card p-4" style={{ borderLeft: `3px solid ${phColor}` }}>
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        {ph.dominant_emotion && <span className="px-2 py-0.5 rounded text-[9px] font-mono capitalize" style={{ background: `${phColor}18`, color: phColor }}>{ph.dominant_emotion}</span>}
                      </div>
                      <p className="text-xs leading-relaxed mb-2" style={{ color: "var(--muted)" }}>{ph.description}</p>
                      {ph.shift_trigger && <p className="text-[10px] italic" style={{ color: "var(--muted-2)" }}>Shift: {ph.shift_trigger}</p>}
                    </div>
                  </motion.div>
                );
              })}
      ...
      {peaks && (
        <Section title="Peak Moments" iconName="sparkle" iconColor="var(--primary)">
          <div className="space-y-3">
            {peaks.highest_point?.description && (
              <div className="p-3 rounded" style={{ background: "var(--surface-alt)", borderLeft: "3px solid var(--green)" }}>
                ...
                {peaks.highest_point.why_it_mattered && <p className="text-xs italic" style={{ color: "var(--muted)" }}>{peaks.highest_point.why_it_mattered}</p>}
              </div>
            )}
            {peaks.lowest_point?.description && (
              <div className="p-3 rounded" style={{ background: "var(--surface-alt)", borderLeft: "3px solid var(--red)" }}>
                ...
                {peaks.lowest_point.what_it_revealed && <p className="text-xs italic" style={{ color: "var(--muted)" }}>{peaks.lowest_point.what_it_revealed}</p>}
              </div>
            )}
      ...
      {topics.length > 0 && (
        <Section title="What You Mostly Talk About" iconName="bubbles">
          <div className="space-y-3">
            {topics.map((t: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }} className="flex items-start gap-3 p-3 rounded" style={{ background: "var(--surface-alt)" }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    ...
                    <span className="px-2 py-0.5 rounded text-[9px] font-mono capitalize" style={{ background: `${SENT_C[t.sentiment] ?? "var(--muted)"}18`, color: SENT_C[t.sentiment] ?? "var(--muted)" }}>{t.sentiment}</span>
      ...
      {roast && (
        <Section title="The Roast" iconName="zap" iconColor="#ff6b35">
          <p className="text-xs mb-4" style={{ color: "var(--muted)" }}>Brutal honesty wrapped in love.</p>
          <div className="space-y-3 mb-4">
            {[{ l: speakerA, v: roast.person_a_roast, c: "var(--primary)" }, { l: speakerB, v: roast.person_b_roast, c: "var(--green)" }, { l: "The Relationship", v: roast.relationship_roast, c: "var(--amber)" }].filter((r) => r.v).map((r) => (
          ...
          {roast.roast_verdict && (
            <div className="p-4 rounded text-center" style={{ background: "#1e1a2e", border: "1px solid #6c55e0" }}>
              <p className="text-sm font-display font-bold italic" style={{ color: "#fff" }}>&quot;{roast.roast_verdict}&quot;</p>
            </div>
          )}
        </Section>
      )}
      ...
      {astro && (
        <Section title="What the Stars Say" iconName="star" iconColor="#9b8cf0">
          ...
          {astro.element_dynamic && (
            <div className="p-3 rounded mb-3 flex items-start gap-3" style={{ background: "var(--pri-soft)", border: "1px solid var(--pri-border)" }}>
              <Icon name="zap" size={14} color="var(--primary)" />
              <div><p className="label mb-0.5" style={{ fontSize: "9px", color: "var(--primary)" }}>ELEMENT DYNAMIC</p><p className="text-sm" style={{ color: "var(--text)" }}>{astro.element_dynamic}</p></div>
            </div>
          )}
          <div className="space-y-3 mb-4">
            {[
              { icon: "activity", label: "Mercury — Communication", v: astro.mercury_reading },
              { icon: "heart", label: "Venus — Love Style", v: astro.venus_reading },
              { icon: "compass", label: "Cosmic Compatibility", v: astro.cosmic_compatibility },
              { icon: "shield", label: "Saturn — The Lesson", v: astro.saturn_truth },
            ].filter((r) => r.v).map((r) => (
              <div key={r.label} className="p-3 rounded" style={{ background: "var(--surface-alt)" }}>
                <div className="flex items-center gap-2 mb-1"><Icon name={r.icon} size={13} color="var(--primary)" /><p className="label" style={{ fontSize: "9px", color: "var(--primary)" }}>{r.label.toUpperCase()}</p></div>
          ...
          {astro.cosmic_verdict && (
            <div className="p-4 rounded text-center" style={{ background: "linear-gradient(135deg,#1e1a2e,#2d1b69)", border: "1px solid #6c55e0" }}>
              ...
              <p className="text-sm font-display font-bold italic" style={{ color: "#fff" }}>&quot;{astro.cosmic_verdict}&quot;</p>
            </div>
          )}
        </Section>
      )}
      ...
      {redFlags.length > 0 && (
        <Section title="Red Flags" iconName="flag" iconColor="var(--red)">
          <div className="space-y-3">
            {redFlags.map((f: any, i: number) => (
              <div key={i} className="p-3 rounded" style={{ background: "var(--red-soft)", border: "1px solid rgba(204,0,0,.15)" }}>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  {f.severity && <span className="px-2 py-0.5 rounded text-[9px] font-mono capitalize" style={{ background: "rgba(204,0,0,.1)", color: "var(--red)" }}>{f.severity}</span>}
                </div>
      ...
      {greenFlags.length > 0 && (
        <Section title="Green Flags" iconName="checkCircle" iconColor="var(--green)">
      ...
      {hi && (
        <Section title="Relationship Health" iconName="heart" iconColor="var(--primary)">
          <div className="space-y-4">
            {[
              { l: "Mutual Respect", v: hi.mutual_respect }, { l: "Emotional Safety", v: hi.emotional_safety },
              { l: "Authenticity", v: hi.authenticity }, { l: "Reciprocity", v: hi.reciprocity }, { l: "Growth Potential", v: hi.growth_potential },
            ].filter((i) => i.v != null).map((i, idx) => {
      ...
      {comm && (
        <Section title="Communication Breakdown" iconName="activity">
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[{ l: "Who Initiates", v: comm.who_initiates_more }, { l: "Split", v: comm.initiation_percentage }, { l: "Humor Level", v: comm.humor_level }, { l: "Power Dynamic", v: comm.power_dynamic }].filter((i) => i.v).map((i) => (
          ...
          <div className="space-y-2">
            {[{ l: `${speakerA}'s Style`, v: comm.response_style_a }, { l: `${speakerB}'s Style`, v: comm.response_style_b }, { l: "Conflict Style", v: comm.conflict_style }, { l: "Affection Shown", v: comm.affection_shown }].filter((i) => i.v).map((i) => (
              <div key={i.l} className="p-3 rounded" style={{ background: "var(--surface-alt)" }}>
                <p className="label mb-1" style={{ fontSize: "9px" }}>{i.l.toUpperCase()}</p>
      ...
      <Section title="Toxicity Analysis" iconName="alertTri" iconColor="var(--amber)">
      ...
      <Section title="Ghosting Risk" iconName="userX" iconColor="var(--muted)">
        <div className="flex items-start gap-3 mb-4">
      ...
      <Section title="Attachment Style" iconName="link">
        <p className="font-display font-bold capitalize mb-2" style={{ color: "var(--text)" }}>{s.attachment_style ?? "Secure"} Attachment</p>
