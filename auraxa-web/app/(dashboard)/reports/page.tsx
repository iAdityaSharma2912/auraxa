  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4" style={{ background: "#fff0f0" }}>
        ...
        <p className="text-sm mb-6 leading-relaxed" style={{ color: "var(--muted)" }}>
          This permanently removes the analysis, scores, timeline, and AI advisor history. Cannot be undone.
        </p>
...
function ReportCard({
  ...
  const name = `${analysis.speakers?.a ?? "You"} & ${analysis.speakers?.b ?? "Them"}`;
  const date = new Date(analysis.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" });
  const variant = analysis.card_variant ?? "mid";
  const variantColor = VARIANT_COLOR[variant] ?? "var(--primary)";
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="min-w-0 flex-1">
              <Link href={`/results/${analysis.id}`} className="hover:text-primary transition-colors">
                <p className="font-display text-sm font-bold" style={{ color: "var(--text)" }}>{name}</p>
              </Link>
              <p className="text-[10px] font-mono mt-0.5" style={{ color: "var(--muted-2)" }}>
                {date}
                {analysis.message_count ? ` · ${analysis.message_count.toLocaleString()} msgs` : ""}
              </p>
            </div>

            {/* Score + delete */}
            <div className="flex items-center gap-2 flex-shrink-0 ml-2">
              {analysis.status === "completed" && s ? (
                <div
                  className="w-9 h-9 rounded flex items-center justify-center"
                  style={{ background: `${variantColor}18`, border: `1px solid ${variantColor}30` }}
                >
                  <span className="font-display text-sm font-bold" style={{ color: variantColor }}>
                    {s.overall_score}
                  </span>
                </div>
              ) : (
                <span
                  ...
                  style={{ background: "var(--pri-soft)", color: "var(--primary)" }}
                >
                  {analysis.status}
                </span>
              )}
          ...
          {s && (
            <div className="grid grid-cols-3 gap-1.5 mb-2">
              {[
                ...
                { l: "Toxicity", v: s.toxicity_level,            c: STATUS_COLOR[s.toxicity_level] ?? "var(--muted)" },
                { l: "Ghost",    v: s.ghosting_risk,             c: STATUS_COLOR[s.ghosting_risk]  ?? "var(--muted)" },
              ].map((m) => (
          ...
          {/* Gen Z verdict */}
          {analysis.genz_verdict && (
            <p className="text-[10px] italic mb-2 leading-relaxed" style={{ color: "var(--muted)" }}>
              &quot;{analysis.genz_verdict}&quot;
            </p>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            {!selectMode && (
              <button
                onClick={onDelete}
                className="sm:hidden label"
                style={{ fontSize: "9px", color: "var(--red)", fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.06em" }}
              >
                DELETE
              </button>
            )}
...
export default function ReportsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setAnalyses(r.data?.analyses ?? r.data ?? []);
    } catch {
    ...
    <div className="px-3 sm:px-5 md:px-8 py-4 md:py-8 max-w-4xl">
      ...
      >
        <div>
          ...
          <h1 className="font-display font-bold" style={{ fontSize: "clamp(1.3rem,5vw,1.875rem)", color: "var(--text)" }}>
            All Reports
          </h1>
        </div>
        ...
        {selectMode && (
          ...
          >
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                onClick={() => setSelected(new Set(analyses.map((a) => a.id)))}
                className="label hover:text-primary transition-colors"
                style={{ fontSize: "9px" }}
              >
                Select all
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center">
          <div className="w-14 h-14 rounded mx-auto mb-4 flex items-center justify-center text-2xl" style={{ background: "var(--surface-alt)", color: "var(--primary)" }}>
            ▤
          </div>
